...
{/*{{{*/
  ...
  if (this->timestamp_div > -1)
  {
    ...
    if (this->timestamp_div > 0)
    {
      ...
      // - scan queue for latest record index -
      last_ts_trace_idx = trace_queue_s_scan_for_last(&this->ts_trace_queue);
    ...
    if (timestamp_last_id < this->trace_last_id)
    {
      ...
      do {
        ...
        trace_s_update_timestamp_structures(this,&timestamp);
...
{/*{{{*/
  ...
  trace_record_timestamp_queue_s_insert(&this->timestamp_queue,a_timestamp);
  ...
  // - insert timestamp to timestamp tree -
  trace_record_timestamp_tree_s_insert(&this->timestamp_tree,a_timestamp);
}/*}}}*/
...
int trace_s_write_timestamp_queue(trace_s *this,time_s a_time)
{/*{{{*/
  ...
  {
    // - drop tail record -
    trace_queue_s_next(&this->ts_trace_queue);
  }

  // - insert record -
  trace_queue_s_insert_blank(&this->ts_trace_queue);
  trace_record_s *ts_trace_record = trace_queue_s_last(&this->ts_trace_queue);
  ...
  // - sync data to storage -
  if (trace_record_s_msync(ts_trace_record,this->ts_trace_queue.rec_size))
  {
    throw_error(TRACE_RECORD_MSYNC_ERROR);
  }
...
{/*{{{*/
  ...
  if (this->timestamp_div > -1)
  {
    ...
    {
      if (trace_s_write_timestamp_queue(this,a_time))
      {
        throw_error(TRACE_WRITE_TIMESTAMP_QUEUE_ERROR);
      }
    }
  }
